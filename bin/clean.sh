...
########Extract Number of nodes and, min and max per Active
	if [ $current_line -eq 1 ]; then
		...
		#messaging delay
		messaging_delay=$(echo $line | cut -f2 -d" ")
		#convert it to an integer
  		let messaging_delay=$messaging_delay+0
...
do
	...
	ssh -o StrictHostKeyChecking=no $netid@$host.utdallas.edu "ps -u $USER | grep java | tr -s ' ' | cut -f1 -d' ' | xargs kill" &
        ssh -o StrictHostKeyChecking=no $netid@$host.utdallas.edu "ps -fu $USER | grep java | tr -s ' ' | cut -f2 -d' ' | xargs kill" & 
