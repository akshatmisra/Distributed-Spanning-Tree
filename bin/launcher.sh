...
do
	...
	portNumber=${port[$node_id]}
	#echo $netid@$host "'$node_id' '$host' '$portNumber' '$nodes_location' '$neighbors' '$root_node' "
	cd $(pwd)
	javac -cp .  $PROG.java 
	cd -
	ssh -o StrictHostKeyChecking=no $netid@$host.utdallas.edu " cd $(pwd); java $PROG '$node_id' '$host' '$portNumber' '$root_node' '$neighbors' '$nodes_location'" &  
